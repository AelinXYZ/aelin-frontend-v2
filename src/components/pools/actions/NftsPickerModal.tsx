    ...
    switch (nftWhitelistProcess) {
      case NftWhitelistProcess.limitedPerWallet:
        for (const collectionAddress in whitelistRules) {
          if (
            nfts.findIndex(
              (nft) => nft.collectionAddress === collectionAddress && nft.isSelected,
            ) !== -1
          ) {
            allocation += whitelistRules[collectionAddress].allocationAmount
          }
        }
      ...
      case NftWhitelistProcess.limitedPerNft:
        for (const collectionAddress in whitelistRules) {
          for (const nft of nfts) {
            if (nft.collectionAddress === collectionAddress && nft.isSelected) {
              allocation += whitelistRules[collectionAddress].allocationAmount
            }
          }
        }

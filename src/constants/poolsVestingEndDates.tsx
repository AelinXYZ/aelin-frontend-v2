export const PoolsVestingEndDates: { [poolAddress: string]: Date } = {
  ['0xabab6d9dd92645789d52fcb5c4988b7ab8d3e4ca']: new Date(0),
  ['0x20369baa917bd1867bdafc24d72458ac777c9a2c']: new Date(0),
  ['0xc7bbd38d1ae4a4aa4bf0d0e7b061b1bb858b0d09']: new Date(0),
  ['0x576c9fb6c46abb24530cea6b7eb51277196575ea']: new Date(1662658841 * 1000),
  ['0xe361ac500fc1d91d49e2c0204963f2cadbcaf67a']: new Date(1663286395 * 1000),
  ['0x21f4f88a95f656ef4ee1ea107569b3b38cf8daef']: new Date(Date.UTC(2023, 4, 14)),
  ['0xd5541dad40ee8e9357606e409d9d3530c0696ede']: new Date(Date.UTC(2023, 5, 11)),
  ['0xc4b35b8bb40368996add8bfc954e3f16579ae82b']: new Date(Date.UTC(2023, 5, 22)),
  ['0x89ff4fbec2081079fa9fa5064ce1c39203165821']: new Date(Date.UTC(2023, 5, 22)),
  ['0x946a90c103336b7d06ee1d1764c7c89aacaecb1a']: new Date(Date.UTC(2023, 6, 31)),
  ['0xedc4ea7043fb6b1d3f9c5732abd4d3aa37269646']: new Date(Date.UTC(2023, 4, 13)),
  ['0x08703a9167a05c99eeb7f9c523a2b073fbc0f28e']: new Date(Date.UTC(2023, 4, 10)),
} as const
